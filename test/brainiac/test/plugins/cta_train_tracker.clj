...
(deftest test-transform
    (alter-var-root (var now) (fn [_] (constantly 1326165484471)))
    (let [result (transform sample-xml)]
      (testing "sets type"
        (is (= "list" (:type result))))

      (testing "sets name"
        (is (="train-tracker" (:name result))))

      (testing "sets title"
        (is (= "UIC-Halsted (Blue)" (:title result))))

      (testing "sets data"
        (is (= ["Forest Park 5 min" "O'Hare 9 min" "Forest Park 18 min" "O'Hare 18 min"] (:data result))))))
